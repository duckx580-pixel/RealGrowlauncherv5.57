package androidx.compose.material.icons.filled;

import bj.n;
import g1.m0;
import g1.t;
import j0.a;
import k0.b;
import k0.d;
import k1.e;
import k1.f;
import k1.h0;
import kotlin.jvm.internal.l;

/* JADX INFO: compiled from: r8-map-id-cf66f36031e302b9a7b5a76da57d208ae1b6ba4a38bba3bee1d4ed6983c54df3 */
/* JADX INFO: loaded from: classes.dex */
public final class ContentPasteGoKt {
    private static f _contentPasteGo;

    public static final f getContentPasteGo(a aVar) {
        l.f("<this>", aVar);
        f fVar = _contentPasteGo;
        if (fVar != null) {
            return fVar;
        }
        e eVar = new e("Filled.ContentPasteGo", 24.0f, 24.0f, 24.0f, 24.0f, 0L, 0, false, 224);
        int i10 = h0.f9067a;
        long j = t.f6905b;
        m0 m0Var = new m0(j);
        n nVarC = gb.e.c(5.0f, 5.0f, 2.0f, 3.0f, 10.0f);
        b.n(nVarC, 5.0f, 2.0f, 6.0f, 2.0f);
        nVarC.s(5.0f);
        nVarC.i(0.0f, -1.1f, -0.9f, -2.0f, -2.0f, -2.0f);
        nVarC.k(-4.18f);
        nVarC.h(14.4f, 1.84f, 13.3f, 1.0f, 12.0f, 1.0f);
        nVarC.p(9.6f, 1.84f, 9.18f, 3.0f);
        nVarC.j(5.0f);
        nVarC.h(3.9f, 3.0f, 3.0f, 3.9f, 3.0f, 5.0f);
        nVarC.t(14.0f);
        nVarC.i(0.0f, 1.1f, 0.9f, 2.0f, 2.0f, 2.0f);
        d.y(nVarC, 5.0f, -2.0f, 5.0f, 5.0f);
        nVarC.n(12.0f, 3.0f);
        nVarC.i(0.55f, 0.0f, 1.0f, 0.45f, 1.0f, 1.0f);
        nVarC.q(-0.45f, 1.0f, -1.0f, 1.0f);
        nVarC.q(-1.0f, -0.45f, -1.0f, -1.0f);
        nVarC.p(11.45f, 3.0f, 12.0f, 3.0f);
        nVarC.g();
        e.a(eVar, nVarC.f3166a, 0, m0Var);
        m0 m0Var2 = new m0(j);
        n nVarB = gb.e.b(18.01f, 13.0f, -1.42f, 1.41f);
        nVarB.m(1.58f, 1.58f);
        nVarB.m(-6.17f, 0.0f);
        nVarB.m(0.0f, 2.0f);
        nVarB.m(6.17f, 0.0f);
        nVarB.m(-1.58f, 1.59f);
        k0.a.k(nVarB, 1.42f, 1.41f, 3.99f, -4.0f);
        e.a(eVar, nVarB.f3166a, 0, m0Var2);
        f fVarB = eVar.b();
        _contentPasteGo = fVarB;
        return fVarB;
    }
}
