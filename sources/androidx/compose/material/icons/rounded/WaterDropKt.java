package androidx.compose.material.icons.rounded;

import bj.n;
import g1.m0;
import g1.t;
import j0.c;
import k0.a;
import k1.e;
import k1.f;
import k1.h0;
import kotlin.jvm.internal.l;

/* JADX INFO: compiled from: r8-map-id-cf66f36031e302b9a7b5a76da57d208ae1b6ba4a38bba3bee1d4ed6983c54df3 */
/* JADX INFO: loaded from: classes.dex */
public final class WaterDropKt {
    private static f _waterDrop;

    public static final f getWaterDrop(c cVar) {
        l.f("<this>", cVar);
        f fVar = _waterDrop;
        if (fVar != null) {
            return fVar;
        }
        e eVar = new e("Rounded.WaterDrop", 24.0f, 24.0f, 24.0f, 24.0f, 0L, 0, false, 224);
        int i10 = h0.f9067a;
        m0 m0Var = new m0(t.f6905b);
        n nVarA = a.a(12.66f, 2.58f);
        nVarA.i(-0.38f, -0.33f, -0.95f, -0.33f, -1.33f, 0.0f);
        nVarA.h(6.45f, 6.88f, 4.0f, 10.62f, 4.0f, 13.8f);
        nVarA.i(0.0f, 4.98f, 3.8f, 8.2f, 8.0f, 8.2f);
        nVarA.q(8.0f, -3.22f, 8.0f, -8.2f);
        nVarA.h(20.0f, 10.62f, 17.55f, 6.88f, 12.66f, 2.58f);
        nVarA.g();
        nVarA.n(7.83f, 14.0f);
        nVarA.i(0.37f, 0.0f, 0.67f, 0.26f, 0.74f, 0.62f);
        nVarA.i(0.41f, 2.22f, 2.28f, 2.98f, 3.64f, 2.87f);
        nVarA.i(0.43f, -0.02f, 0.79f, 0.32f, 0.79f, 0.75f);
        nVarA.i(0.0f, 0.4f, -0.32f, 0.73f, -0.72f, 0.75f);
        nVarA.i(-2.13f, 0.13f, -4.62f, -1.09f, -5.19f, -4.12f);
        nVarA.h(7.01f, 14.42f, 7.37f, 14.0f, 7.83f, 14.0f);
        nVarA.g();
        e.a(eVar, nVarA.f3166a, 0, m0Var);
        f fVarB = eVar.b();
        _waterDrop = fVarB;
        return fVarB;
    }
}
