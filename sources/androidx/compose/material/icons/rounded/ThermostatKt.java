package androidx.compose.material.icons.rounded;

import bj.n;
import g1.m0;
import g1.t;
import j0.c;
import k0.a;
import k0.b;
import k1.e;
import k1.f;
import k1.h0;
import kotlin.jvm.internal.l;

/* JADX INFO: compiled from: r8-map-id-cf66f36031e302b9a7b5a76da57d208ae1b6ba4a38bba3bee1d4ed6983c54df3 */
/* JADX INFO: loaded from: classes.dex */
public final class ThermostatKt {
    private static f _thermostat;

    public static final f getThermostat(c cVar) {
        l.f("<this>", cVar);
        f fVar = _thermostat;
        if (fVar != null) {
            return fVar;
        }
        e eVar = new e("Rounded.Thermostat", 24.0f, 24.0f, 24.0f, 24.0f, 0L, 0, false, 224);
        int i10 = h0.f9067a;
        m0 m0Var = new m0(t.f6905b);
        n nVarC = a.c(15.0f, 13.0f, 15.0f, 5.0f);
        nVarC.i(0.0f, -1.66f, -1.34f, -3.0f, -3.0f, -3.0f);
        nVarC.p(9.0f, 3.34f, 9.0f, 5.0f);
        nVarC.t(8.0f);
        nVarC.i(-1.21f, 0.91f, -2.0f, 2.37f, -2.0f, 4.0f);
        nVarC.i(0.0f, 2.76f, 2.24f, 5.0f, 5.0f, 5.0f);
        nVarC.q(5.0f, -2.24f, 5.0f, -5.0f);
        nVarC.i(0.0f, -1.63f, -0.79f, -3.09f, -2.0f, -4.0f);
        a.t(nVarC, 13.0f, 11.0f, -2.0f);
        nVarC.l(11.0f, 5.0f);
        nVarC.i(0.0f, -0.55f, 0.45f, -1.0f, 1.0f, -1.0f);
        nVarC.q(1.0f, 0.45f, 1.0f, 1.0f);
        nVarC.k(-0.5f);
        nVarC.i(-0.28f, 0.0f, -0.5f, 0.22f, -0.5f, 0.5f);
        nVarC.q(0.22f, 0.5f, 0.5f, 0.5f);
        b.v(nVarC, 0.5f, 2.0f, -0.5f);
        nVarC.i(-0.28f, 0.0f, -0.5f, 0.22f, -0.5f, 0.5f);
        nVarC.q(0.22f, 0.5f, 0.5f, 0.5f);
        a.i(nVarC, 0.5f, 2.0f);
        e.a(eVar, nVarC.f3166a, 0, m0Var);
        f fVarB = eVar.b();
        _thermostat = fVarB;
        return fVarB;
    }
}
