package com.facebook.ads.redexgen.X;

import android.content.Context;
import android.graphics.Bitmap;
import android.graphics.drawable.BitmapDrawable;
import android.graphics.drawable.Drawable;
import android.graphics.drawable.LayerDrawable;
import android.os.Build;
import android.text.TextUtils;
import android.util.Log;
import android.view.View;
import android.view.ViewGroup;
import android.widget.FrameLayout;
import android.widget.ImageView;
import androidx.annotation.Nullable;
import com.facebook.ads.AdSettings;
import com.facebook.ads.ExtraHints;
import com.facebook.ads.MediaView;
import com.facebook.ads.NativeAd;
import com.facebook.ads.NativeAdBase;
import com.facebook.ads.NativeAdLayout;
import com.facebook.ads.NativeAdListener;
import com.facebook.ads.NativeBannerAd;
import com.facebook.ads.internal.api.AdNativeComponentView;
import com.facebook.ads.internal.api.NativeAdBaseApi;
import com.facebook.ads.internal.api.NativeAdImageApi;
import com.facebook.ads.internal.context.Repairable;
import com.facebook.ads.internal.protocol.AdErrorType;
import com.facebook.ads.internal.protocol.AdPlacementType;
import com.facebook.ads.internal.settings.AdInternalSettings;
import com.facebook.proguard.annotations.DoNotStrip;
import com.rtsoft.growtopia.R;
import com.usercentrics.sdk.extensions.TimeExtensionsKt;
import java.lang.ref.WeakReference;
import java.lang.reflect.Proxy;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.Iterator;
import java.util.List;
import java.util.Locale;
import java.util.UUID;
import java.util.WeakHashMap;

/* JADX INFO: loaded from: assets/audience_network.dex */
public final class TB implements com.facebook.ads.Ad, NativeAdBaseApi, Repairable, InterfaceC0469Iz {
    public static C6G A0k;
    public static byte[] A0l;
    public static String[] A0m = {"Bl7Z7O36thNMsAQG86cT5hXAswukL2TU", "w0l3kgVUfrf8f1EzcfbBJ9JqF1JIQyF1", "3FYeVZ4P9Hva0brxRfDXnSutu97odoaf", "TkemScCQ72wM3yeu7V5OcaIZTlgQuG0T", "aV5nfdXEnoj7uvLi", "brDFWAS69jf159nGVdOYVvf", "WGIwYSb5Wjcv", "W7skGFSknqdZoaqu210O1mF"};
    public static final String A0n;
    public static final WeakHashMap<View, WeakReference<TB>> A0o;
    public long A00;

    @Nullable
    public Drawable A01;

    @Nullable
    public View.OnTouchListener A02;

    @Nullable
    public View A03;

    @Nullable
    public View A04;

    @Nullable
    public View A05;

    @Nullable
    public View A06;

    @Nullable
    public NativeAdLayout A07;
    public EnumC00330z A08;

    @Nullable
    public ZF A09;

    @Nullable
    public EB A0A;

    @Nullable
    public C0859Yk A0B;

    @Nullable
    public C6L A0C;

    @Nullable
    public AnonymousClass84 A0D;
    public EnumC0455Il A0E;

    @Nullable
    public TD A0F;

    @Nullable
    public TA A0G;

    @Nullable
    public C0463It A0H;
    public EnumC0464Iu A0I;
    public J8 A0J;

    @Nullable
    public M3 A0K;

    @Nullable
    public N0 A0L;

    @Nullable
    @DoNotStrip
    public O6 A0M;

    @Nullable
    public C0599Oe A0N;

    @Nullable
    @DoNotStrip
    public AbstractC0639Ps A0O;

    @Nullable
    @DoNotStrip
    public AbstractC0639Ps A0P;

    @Nullable
    public C0640Pt A0Q;

    @Nullable
    public C0640Pt A0R;

    @Nullable
    public String A0S;

    @Nullable
    public String A0T;
    public WeakReference<C0832Xh> A0U;

    @Nullable
    public WeakReference<AbstractC0639Ps> A0V;
    public boolean A0W;
    public boolean A0X;
    public boolean A0Y;
    public boolean A0Z;

    @Nullable
    public ZO A0a;
    public final C6G A0b;
    public final C0823Wy A0c;

    @Nullable
    public final InterfaceC0460Iq A0d;
    public final J1 A0e;
    public final L6 A0f;
    public final String A0g;
    public final String A0h;
    public final List<View> A0i;
    public volatile boolean A0j;

    public static String A0W(int i10, int i11, int i12) {
        byte[] bArrCopyOfRange = Arrays.copyOfRange(A0l, i10, i10 + i11);
        for (int i13 = 0; i13 < bArrCopyOfRange.length; i13++) {
            bArrCopyOfRange[i13] = (byte) ((bArrCopyOfRange[i13] - i12) - 40);
        }
        return new String(bArrCopyOfRange);
    }

    public static void A0d() {
        A0l = new byte[]{86, -112, -100, -107, -108, -88, -92, -84, -88, -38, -38, -92, -85, -75, -71, -27, -30, -29, -70, -79, -78, -38, -34, 11, 9, -34, -33, 9, -40, 125, -96, 92, -86, -85, -80, 92, -88, -85, -99, -96, -95, -96, -67, -32, -48, -31, -23, -20, -24, -35, -16, -31, -100, -93, -95, -17, -93, -100, -27, -17, -100, -22, -21, -16, -100, -35, -100, -22, -35, -16, -27, -14, -31, -100, -35, -32, -105, -109, -110, -58, -75, -70, -74, -65, -76, -74, -97, -74, -59, -56, -64, -61, -68, -83, -56, -48, -45, -52, -53, -121, -37, -42, -121, -45, -42, -56, -53, -121, -76, -52, -53, -48, -56, -107, -96, -69, -61, -58, -65, -66, 122, -50, -55, 122, -55, -68, -50, -69, -61, -56, 122, -50, -65, -57, -54, -58, -69, -50, -65, 122, -93, -98, 122, -64, -52, -55, -57, 122, -68, -61, -66, 122, -54, -69, -45, -58, -55, -69, -66, 122, -127, 127, -51, -127, -79, -42, -36, -51, -38, -42, -55, -44, -120, -51, -38, -38, -41, -38, -106, 114, -47, -10, -2, -23, -12, -15, -20, -88, -5, -19, -4, -88, -9, -18, -88, -21, -12, -15, -21, -13, -23, -22, -12, -19, -88, -2, -15, -19, -1, -5, -73, -49, -50, -45, -53, -64, -45, -49, -31, -118, -48, -39, -36, -118, -45, -51, -39, -40, -118, -45, -35, -118, -45, -35, -118, -41, -45, -35, -35, -45, -40, -47, -104, -71, -47, -48, -43, -51, -62, -43, -47, -29, -116, -43, -33, -116, -39, -43, -33, -33, -43, -38, -45, -102, -117, -77, -79, -78, 94, -82, -80, -83, -76, -89, -94, -93, 94, -97, 94, -108, -89, -93, -75, -49, -30, -11, -22, -9, -26, -95, -62, -27, -95, -8, -30, -12, -95, -30, -19, -13, -26, -30, -27, -6, -95, -13, -26, -24, -22, -12, -11, -26, -13, -26, -27, -95, -8, -22, -11, -23, -95, -30, -95, -41, -22, -26, -8, -81, -95, -62, -10, -11, -16, -95, -10, -17, -13, -26, -24, -22, -12, -11, -26, -13, -22, -17, -24, -95, -30, -17, -27, -95, -15, -13, -16, -28, -26, -26, -27, -22, -17, -24, -81, -115, -96, -77, -88, -75, -92, 95, -96, -93, 95, -93, -92, -78, -77, -79, -82, -72, -92, -93, -40, -21, -2, -13, 0, -17, -86, -21, -18, -86, -10, -7, -21, -18, -86, -4, -17, -5, -1, -17, -3, -2, -17, -18, -115, -96, -100, -82, 87, -104, -93, -87, -100, -104, -101, -80, 87, -87, -100, -98, -96, -86, -85, -100, -87, -100, -101, 87, -82, -96, -85, -97, 87, -104, 87, -123, -104, -85, -96, -83, -100, 120, -101, 101, 87, 120, -84, -85, -90, 87, -84, -91, -87, -100, -98, -96, -86, -85, -100, -87, -96, -91, -98, 87, -104, -91, -101, 87, -89, -87, -90, -102, -100, -100, -101, -96, -91, -98, 101, -58, -39, -43, -25, -112, -34, -33, -28, -112, -30, -43, -41, -39, -29, -28, -43, -30, -43, -44, -112, -25, -39, -28, -40, -112, -28, -40, -39, -29, -112, -66, -47, -28, -39, -26, -43, -79, -44, -104, -101, 87, -92, -100, -101, -96, -104, 87, -85, -80, -89, -100, 87, -96, -86, 87, -91, -90, -85, 87, -86, -84, -89, -89, -90, -87, -85, -100, -101, 101, -116, -101, -108, -109, -108, -94, -93, -95, -98, -88, -94, -91, -105, -102, 119, -102, 7, 10, -4, -1, -36, -1, -61, -60, -69, -2, -4, 7, 7, 0, -1, -69, 8, 10, 13, 0, -69, 15, 3, -4, 9, -69, 10, 9, -2, 0, -17, -30, -11, -22, -9, -26};
    }

    /* JADX WARN: Failed to parse debug info
    java.lang.ArrayIndexOutOfBoundsException: Index 24 out of bounds for length 17
    	at jadx.plugins.input.dex.sections.debuginfo.DebugInfoParser.startVar(DebugInfoParser.java:203)
    	at jadx.plugins.input.dex.sections.debuginfo.DebugInfoParser.process(DebugInfoParser.java:135)
    	at jadx.plugins.input.dex.sections.DexCodeReader.getDebugInfo(DexCodeReader.java:122)
    	at jadx.core.dex.nodes.MethodNode.getDebugInfo(MethodNode.java:645)
    	at jadx.core.dex.visitors.debuginfo.DebugInfoAttachVisitor.visit(DebugInfoAttachVisitor.java:38)
     */
    private void A0f(View view, View view2, List<View> list, boolean z3) {
        TA ta;
        if (!A0q()) {
            this.A0c.A0D().ADf();
        }
        if (view == null) {
            String strA0W = A0W(264, 19, 22);
            if (!A0q()) {
                this.A0c.A0D().ADe(strA0W);
            }
            throw new IllegalArgumentException(strA0W);
        }
        if (list == null || list.size() == 0) {
            String strA0W2 = A0W(180, 30, 96);
            if (!A0q()) {
                this.A0c.A0D().ADe(strA0W2);
            }
            throw new IllegalArgumentException(strA0W2);
        }
        ZO zoA0C = A0C();
        if (zoA0C == null) {
            String strA0W3 = A0W(29, 13, 20);
            if (!A0q()) {
                this.A0c.A0D().ADe(strA0W3);
            }
            Log.e(A0n, strA0W3);
            J3 j32 = new J3(AdErrorType.NATIVE_AD_IS_NOT_LOADED, strA0W3);
            A11().A0D().A2a(L5.A01(this.A00), j32.A03().getErrorCode(), j32.A04());
            if (!ID.A23(this.A0c) || (ta = this.A0G) == null) {
                return;
            }
            ta.AAc(j32);
            return;
        }
        String str = this.A0T;
        boolean z10 = view instanceof FrameLayout;
        String[] strArr = A0m;
        if (strArr[7].length() != strArr[5].length()) {
            throw new RuntimeException();
        }
        String[] strArr2 = A0m;
        strArr2[0] = "EctJGKY0XxsuqG0UQd3u1zsk6i1LiKEB";
        strArr2[2] = "xfmrtxxFdZMJNwdY4ICwqytBKlocc6OO";
        if (z10 && str != null) {
            A0g((FrameLayout) view, str);
        }
        NativeAdLayout nativeAdLayout = this.A07;
        if (nativeAdLayout != null) {
            ((XQ) nativeAdLayout.getNativeAdLayoutApi()).A02();
        }
        C0832Xh c0832Xh = this.A0U.get();
        if (c0832Xh != null && zoA0C.A08() == 1) {
            c0832Xh.A03(LM.AN_INFO_ICON);
        }
        if (view2 == null) {
            if (this.A0J == J8.A05) {
                AdErrorType adErrorType = AdErrorType.NO_MEDIAVIEW_IN_NATIVEAD;
                String strA0W4 = A0W(243, 21, 68);
                J3 j33 = new J3(adErrorType, strA0W4);
                A11().A0D().A2a(L5.A01(this.A00), j33.A03().getErrorCode(), j33.A04());
                TA ta2 = this.A0G;
                if (ta2 != null) {
                    ta2.AAc(j33);
                }
                if (AdInternalSettings.isDebugBuild()) {
                    Log.e(A0n, strA0W4);
                    return;
                }
                return;
            }
            AdErrorType adErrorType2 = AdErrorType.NO_MEDIAVIEW_IN_NATIVEBANNERAD;
            String strA0W5 = A0W(210, 33, 66);
            J3 j34 = new J3(adErrorType2, strA0W5);
            A11().A0D().A2a(L5.A01(this.A00), j34.A03().getErrorCode(), j34.A04());
            TA ta3 = this.A0G;
            if (ta3 != null) {
                ta3.AAc(j34);
            }
            if (AdInternalSettings.isDebugBuild()) {
                Log.e(A0n, strA0W5);
                return;
            }
            return;
        }
        boolean z11 = (view2 instanceof AdNativeComponentView) && ((AdNativeComponentView) view2).getAdContentsView() != null;
        boolean z12 = z3 && (view2 instanceof ImageView);
        if (!z11 && !z12) {
            if (this.A0G != null) {
                J3 j35 = new J3(AdErrorType.UNSUPPORTED_AD_ASSET_NATIVEAD, A0W(519, 31, 15));
                A11().A0D().A2a(L5.A01(this.A00), j35.A03().getErrorCode(), j35.A04());
                this.A0G.AAc(j35);
                return;
            }
            return;
        }
        if (this.A04 != null) {
            Log.w(A0n, A0W(283, 80, 89));
            String[] strArr3 = A0m;
            if (strArr3[7].length() != strArr3[5].length()) {
                throw new RuntimeException();
            }
            String[] strArr4 = A0m;
            strArr4[7] = "Yitqb4l5O5wHhToyiUZG328";
            strArr4[5] = "n4uNZUYEyoiQJdMJgH58rRy";
            unregisterView();
        }
        if (A0o.containsKey(view) && A0o.get(view).get() != null) {
            Log.w(A0n, A0W(406, 75, 15));
            A0o.get(view).get().unregisterView();
        }
        this.A0F = new TD(this, this.A0c, null);
        this.A04 = view;
        this.A06 = view2;
        if (view instanceof ViewGroup) {
            this.A0K = new M3(this.A0c, new TH(this));
            ((ViewGroup) view).addView(this.A0K);
        }
        if (zoA0C.A0U()) {
            this.A0O = new TG(this);
            this.A0Q = new C0640Pt(view, 1, new WeakReference(this.A0O), this.A0c);
            this.A0Q.A0Y(false);
            this.A0Q.A0X(zoA0C.A09());
            this.A0Q.A0U();
            this.A0c.A0D().A9Y();
        }
        ArrayList arrayList = new ArrayList(list);
        View view3 = this.A05;
        if (view3 != null) {
            arrayList.add(view3);
        }
        Iterator it = arrayList.iterator();
        while (it.hasNext()) {
            A1K((View) it.next());
        }
        int iA00 = A00();
        this.A0P = new TF(this, view2, z12, zoA0C);
        if (view2 instanceof AdNativeComponentView) {
            this.A03 = ((AdNativeComponentView) view2).getAdContentsView();
        } else {
            this.A03 = view2;
        }
        if (ID.A1P(this.A0c)) {
            String strA1B = A1B();
            InterfaceC0449If interfaceC0449IfA09 = A11().A09();
            View view4 = this.A03;
            if (strA1B == null) {
                strA1B = A0W(0, 0, 21);
            }
            interfaceC0449IfA09.AFF(view4, strA1B, this.A03 instanceof P0, true);
        }
        this.A0R = new C0640Pt(this.A03, iA00, A03(), true, new WeakReference(this.A0P), this.A0c);
        this.A0R.A0Y(!A0q());
        this.A0R.A0W(A01());
        this.A0R.A0X(A02());
        View view5 = this.A03;
        if (view5 instanceof C0559Mq) {
            ((C0559Mq) view5).A06(this.A0R);
        }
        this.A09 = new ZF(this.A0c, new TC(this, null), this.A0R, this.A0a);
        this.A09.A0D(arrayList);
        A0o.put(view, new WeakReference<>(this));
        if (Build.VERSION.SDK_INT < 18 || !ID.A0z(this.A0c)) {
            return;
        }
        this.A0L = new N0();
        this.A0L.A0C(this.A0g);
        this.A0L.A0B(this.A0c.getPackageName());
        this.A0L.A0A(this.A0R);
        ZO zo = this.A0a;
        if (zo != null && zo.A0E().A03() > 0) {
            AnonymousClass11 anonymousClass11A0E = this.A0a.A0E();
            this.A0L.A08(anonymousClass11A0E.A03(), anonymousClass11A0E.A04());
        }
        AnonymousClass84 anonymousClass84 = this.A0D;
        if (anonymousClass84 != null) {
            this.A0L.A09(anonymousClass84.A0C());
        } else {
            EB eb2 = this.A0A;
            if (eb2 != null && eb2.A0D() != null) {
                this.A0L.A09(this.A0A.A0D().A0C());
            }
        }
        View view6 = this.A04;
        String[] strArr5 = A0m;
        if (strArr5[4].length() != strArr5[6].length()) {
            String[] strArr6 = A0m;
            strArr6[4] = "DFdj5aRKmThGOYGM";
            strArr6[6] = "4rU7VADNdisg";
            view6.getOverlay().add(this.A0L);
            return;
        }
        String[] strArr7 = A0m;
        strArr7[7] = "HovrQjOKGOU1glve9p8eAJs";
        strArr7[5] = "lnqkcQfVF9X2NaZpGo2NogE";
        view6.getOverlay().add(this.A0L);
    }

    static {
        A0d();
        A0n = TB.class.getSimpleName();
        A0o = new WeakHashMap<>();
    }

    public TB(Context context, String str, @Nullable InterfaceC0460Iq interfaceC0460Iq, boolean z3) {
        this.A0h = UUID.randomUUID().toString();
        this.A0J = J8.A05;
        this.A0E = EnumC0455Il.A04;
        this.A08 = EnumC00330z.A03;
        this.A0U = new WeakReference<>(null);
        this.A0i = new ArrayList();
        this.A0f = new L6();
        this.A0Z = false;
        this.A0Y = false;
        this.A00 = -1L;
        if (context instanceof C0823Wy) {
            this.A0c = (C0823Wy) context;
        } else if (!z3) {
            this.A0c = C5G.A04(context);
        } else {
            this.A0c = C5G.A02(context);
        }
        this.A0c.A0G(this);
        this.A0g = str;
        this.A0d = interfaceC0460Iq;
        C6G c6g = A0k;
        if (c6g != null) {
            this.A0b = c6g;
        } else {
            this.A0b = new C6G(this.A0c);
        }
        this.A05 = new View(context);
        this.A0e = new J1(this.A0c, this);
    }

    public TB(TB tb2) {
        this((Context) tb2.A0c, (String) null, tb2.A0d, true);
        this.A0D = tb2.A0D;
        this.A0a = tb2.A0a;
        this.A0B = tb2.A0B;
        this.A0j = true;
        this.A05 = new View(this.A0c);
    }

    public TB(C0823Wy c0823Wy, ZO zo, @Nullable AnonymousClass84 anonymousClass84, @Nullable InterfaceC0460Iq interfaceC0460Iq) {
        this((Context) c0823Wy, (String) null, interfaceC0460Iq, true);
        this.A0a = zo;
        this.A0D = anonymousClass84;
        this.A0j = true;
        this.A05 = new View(c0823Wy);
    }

    public TB(C0823Wy c0823Wy, ZO zo, @Nullable AnonymousClass84 anonymousClass84, InterfaceC0460Iq interfaceC0460Iq, @Nullable C0859Yk c0859Yk) {
        this(c0823Wy, zo, anonymousClass84, interfaceC0460Iq);
        this.A0B = c0859Yk;
    }

    private int A00() {
        AnonymousClass84 anonymousClass84 = this.A0D;
        if (anonymousClass84 != null) {
            return anonymousClass84.A04();
        }
        EB eb2 = this.A0A;
        if (eb2 == null || eb2.A0D() == null) {
            return 1;
        }
        return this.A0A.A0D().A04();
    }

    private int A01() {
        AnonymousClass84 anonymousClass84 = this.A0D;
        if (anonymousClass84 != null) {
            return anonymousClass84.A07();
        }
        ZO zo = this.A0a;
        if (zo != null) {
            int iA0C = zo.A0C();
            String[] strArr = A0m;
            if (strArr[1].charAt(16) == strArr[3].charAt(16)) {
                throw new RuntimeException();
            }
            String[] strArr2 = A0m;
            strArr2[7] = "EUscbuPuA5cZxxPx7y6ncGc";
            strArr2[5] = "07NrZ6juCFttxvE5hiFlF8c";
            return iA0C;
        }
        EB eb2 = this.A0A;
        if (eb2 != null && eb2.A0D() != null) {
            return this.A0A.A0D().A07();
        }
        return 0;
    }

    private int A02() {
        AnonymousClass84 anonymousClass84 = this.A0D;
        if (anonymousClass84 != null) {
            return anonymousClass84.A08();
        }
        ZO zo = this.A0a;
        if (zo != null) {
            int iA0D = zo.A0D();
            String[] strArr = A0m;
            if (strArr[0].charAt(5) == strArr[2].charAt(5)) {
                throw new RuntimeException();
            }
            String[] strArr2 = A0m;
            strArr2[1] = "wlXrbCoONy6aNPfJSLZ4JGG9g8lWOQWJ";
            strArr2[3] = "zMDzXw8vWIk5v34E7N5RdIcOKoPIh5je";
            return iA0D;
        }
        EB eb2 = this.A0A;
        String[] strArr3 = A0m;
        if (strArr3[4].length() != strArr3[6].length()) {
            String[] strArr4 = A0m;
            strArr4[7] = "IJhE7OuQr9AUmHg0gZAU0NE";
            strArr4[5] = "Tpbr9nnLmrgR8Cas3OKWnYo";
            if (eb2 == null) {
                return TimeExtensionsKt.MILLIS_PER_SECOND;
            }
        } else {
            String[] strArr5 = A0m;
            strArr5[0] = "x7i9CQzVOJPdqJX1dHOWNRL80PPtBuUS";
            strArr5[2] = "xh5QtfSCbDAINUU48h4MV8p0ljd4aaCa";
            if (eb2 == null) {
                return TimeExtensionsKt.MILLIS_PER_SECOND;
            }
        }
        if (eb2.A0D() != null) {
            return this.A0A.A0D().A08();
        }
        return TimeExtensionsKt.MILLIS_PER_SECOND;
    }

    private int A03() {
        AnonymousClass84 anonymousClass84 = this.A0D;
        if (anonymousClass84 != null) {
            return anonymousClass84.A09();
        }
        EB eb2 = this.A0A;
        if (eb2 == null || eb2.A0D() == null) {
            return 0;
        }
        return this.A0A.A0D().A09();
    }

    public static Drawable A05(C0823Wy c0823Wy, Bitmap bitmap, boolean z3, @Nullable String str) {
        BitmapDrawable bitmapDrawableA00;
        BitmapDrawable bitmapDrawable = new BitmapDrawable(c0823Wy.getResources(), bitmap);
        if (z3 && (bitmapDrawableA00 = C0598Od.A00(c0823Wy, str)) != null) {
            Drawable iconViewDrawable = new LayerDrawable(new Drawable[]{bitmapDrawable, bitmapDrawableA00});
            return iconViewDrawable;
        }
        return bitmapDrawable;
    }

    public static NativeAdBase A0A(Context context, String str, String str2) throws J4 {
        J8 j8A00 = JD.A00(str2);
        if (j8A00 != null) {
            J8 j82 = J8.A04;
            String[] strArr = A0m;
            if (strArr[7].length() != strArr[5].length()) {
                throw new RuntimeException();
            }
            String[] strArr2 = A0m;
            strArr2[0] = "9sjWaS9CbZxjWOFrlYfgckD2h0ahWze6";
            strArr2[2] = "CgLfFBC4KJYZcdRIWMiGUMQeYT9XpqO1";
            if (j8A00 == j82) {
                return new NativeBannerAd(context, str);
            }
            J8 template = J8.A05;
            if (j8A00 == template) {
                return new NativeAd(context, str);
            }
            throw new J4(AdErrorType.BID_PAYLOAD_ERROR, String.format(Locale.US, A0W(42, 34, 84), j8A00));
        }
        throw new J4(AdErrorType.BID_PAYLOAD_ERROR, String.format(Locale.US, A0W(R.styleable.AppCompatTheme_windowFixedHeightMajor, 50, 50), str2));
    }

    @Nullable
    private final ZO A0C() {
        ZO zo = this.A0a;
        if (zo != null && zo.A0T()) {
            return zo;
        }
        return null;
    }

    private AnonymousClass11 A0F() {
        return A0G(false);
    }

    private AnonymousClass11 A0G(boolean z3) {
        ZO zo = this.A0a;
        if (zo != null && zo.A0T()) {
            if (z3) {
                this.A0a.A0I();
            }
            ZO zo2 = this.A0a;
            String[] strArr = A0m;
            if (strArr[0].charAt(5) == strArr[2].charAt(5)) {
                throw new RuntimeException();
            }
            String[] strArr2 = A0m;
            strArr2[7] = "bGIdjP48JCmqHZjd8yMcvy0";
            strArr2[5] = "TixE3qv3ULKos9kbBUakVg1";
            return zo2.A0E();
        }
        return new AnonymousClass11();
    }

    public static InterfaceC0460Iq A0K() {
        return new TE();
    }

    public static TB A0L(NativeAdBaseApi nativeAdBaseApi) {
        if (nativeAdBaseApi instanceof Proxy) {
            return (TB) ((AnonymousClass50) Proxy.getInvocationHandler(nativeAdBaseApi)).A04();
        }
        return (TB) nativeAdBaseApi;
    }

    /* JADX INFO: Access modifiers changed from: private */
    @Nullable
    /* JADX INFO: renamed from: A0M, reason: merged with bridge method [inline-methods] */
    public final C0461Ir getAdChoicesIcon() {
        return A0F().A0E();
    }

    /* JADX INFO: Access modifiers changed from: private */
    @Nullable
    /* JADX INFO: renamed from: A0O, reason: merged with bridge method [inline-methods] */
    public final C0462Is getAdStarRating() {
        return A0F().A0H();
    }

    private AdPlacementType A0R() {
        if (this.A0J == J8.A05) {
            return AdPlacementType.NATIVE;
        }
        return AdPlacementType.NATIVE_BANNER;
    }

    private void A0Z() {
        for (View v6 : this.A0i) {
            v6.setOnClickListener(null);
            v6.setOnTouchListener(null);
            v6.setOnLongClickListener(null);
        }
        this.A0i.clear();
    }

    private void A0a() {
        if (!TextUtils.isEmpty(getAdChoicesLinkUrl())) {
            KL.A0E(new KL(), this.A0c, KM.A00(getAdChoicesLinkUrl()), A1B());
        }
    }

    /* JADX INFO: Access modifiers changed from: private */
    public void A0b() {
        this.A0f.A05();
        this.A0e.A05();
    }

    /* JADX INFO: Access modifiers changed from: private */
    public void A0c() {
        C0640Pt c0640Pt = this.A0Q;
        if (c0640Pt != null) {
            c0640Pt.A0V();
            this.A0c.A0D().A9Z();
            this.A0Q = null;
        }
    }

    public static void A0e(@Nullable Drawable drawable, ImageView imageView) {
        imageView.setScaleType(ImageView.ScaleType.FIT_XY);
        if (drawable != null) {
            imageView.setImageDrawable(drawable);
        }
        if (Build.VERSION.SDK_INT >= 16) {
            imageView.setBackground(null);
        } else {
            imageView.setBackgroundDrawable(null);
        }
    }

    private void A0g(FrameLayout frameLayout, String str) {
        C0599Oe c0599Oe = this.A0N;
        if (c0599Oe != null) {
            frameLayout.removeView(c0599Oe);
        }
        this.A0N = C0598Od.A01(C5G.A02(this.A0c), str);
        C0599Oe c0599Oe2 = this.A0N;
        if (c0599Oe2 != null) {
            frameLayout.addView(c0599Oe2, new FrameLayout.LayoutParams(-1, -1));
            frameLayout.bringChildToFront(this.A0N);
        }
    }

    private void A0h(@Nullable ZO zo, boolean z3) {
        if (zo == null) {
            return;
        }
        boolean zEquals = this.A0E.equals(EnumC0455Il.A04);
        String strA0W = A0W(596, 6, 89);
        if (zEquals) {
            AnonymousClass11 anonymousClass11A0E = zo.A0E();
            String clientToken = zo.A63();
            if (!TextUtils.isEmpty(clientToken)) {
                this.A0b.A0d(new C0445Ib(clientToken, this.A0c.A08()));
            }
            if (anonymousClass11A0E.A0G() != null) {
                C6E c6e = new C6E(anonymousClass11A0E.A0G().getUrl(), anonymousClass11A0E.A0G().getHeight(), anonymousClass11A0E.A0G().getWidth(), zo.A0G(), A0W(596, 6, 89));
                c6e.A00 = this.A0C;
                this.A0b.A0V();
                this.A0b.A0b(c6e);
            }
            if (!this.A0J.equals(J8.A04)) {
                if (anonymousClass11A0E.A0F() != null) {
                    this.A0b.A0b(new C6E(anonymousClass11A0E.A0F().getUrl(), anonymousClass11A0E.A0F().getHeight(), anonymousClass11A0E.A0F().getWidth(), zo.A0G(), A0W(596, 6, 89)));
                }
                List<TB> listA0H = zo.A0H();
                String[] strArr = A0m;
                if (strArr[1].charAt(16) == strArr[3].charAt(16)) {
                    throw new RuntimeException();
                }
                String[] strArr2 = A0m;
                strArr2[1] = "BiVeVWtUTguxpSjeMqAkCwQXeKWOVTz4";
                strArr2[3] = "8sdR9nWPZ664Gt9BvBocaDJi5It1hBkX";
                if (listA0H != null) {
                    for (TB tb2 : zo.A0H()) {
                        if (tb2.getAdCoverImage() != null) {
                            this.A0b.A0b(new C6E(tb2.getAdCoverImage().getUrl(), tb2.getAdCoverImage().getHeight(), tb2.getAdCoverImage().getWidth(), zo.A0G(), A0W(596, 6, 89)));
                        }
                    }
                }
                String strA0d = anonymousClass11A0E.A0d();
                if (!TextUtils.isEmpty(strA0d)) {
                    this.A0b.A0a(new C6C(strA0d, zo.A0G(), A0W(596, 6, 89), anonymousClass11A0E.A0A()));
                }
            }
            AnonymousClass18 anonymousClass18A0D = anonymousClass11A0E.A0D();
            if (anonymousClass18A0D != null && anonymousClass18A0D.A0s()) {
                C6C c6c = new C6C(anonymousClass18A0D.A0W(), anonymousClass18A0D.A0a(), strA0W);
                c6c.A04 = true;
                c6c.A03 = A0W(0, 5, 0);
                this.A0b.A0X(c6c);
            }
        }
        this.A0b.A0W(new TI(this, zo, z3), new AnonymousClass69(zo.A0G(), strA0W));
    }

    private void A0i(AnonymousClass12 anonymousClass12) {
        ZO zo = this.A0a;
        if (zo == null) {
            return;
        }
        zo.A0K(anonymousClass12);
    }

    public static void A0j(NativeAdImageApi nativeAdImageApi, ImageView imageView, C0823Wy c0823Wy) {
        if (nativeAdImageApi != null && imageView != null) {
            new S2(imageView, c0823Wy).A05(nativeAdImageApi.getHeight(), nativeAdImageApi.getWidth()).A07(nativeAdImageApi.getUrl());
        }
    }

    private final void A0m(TA ta) {
        this.A0G = ta;
    }

    private final void A0n(String str) {
        this.A0S = str;
    }

    private void A0o(List<View> list, View view) {
        InterfaceC0460Iq interfaceC0460Iq = this.A0d;
        if (interfaceC0460Iq != null && interfaceC0460Iq.AEg(view)) {
            return;
        }
        if (view instanceof ViewGroup) {
            ViewGroup viewGroup = (ViewGroup) view;
            for (int i10 = 0; i10 < viewGroup.getChildCount(); i10++) {
                View childAt = viewGroup.getChildAt(i10);
                String[] strArr = A0m;
                String str = strArr[7];
                String str2 = strArr[5];
                int i11 = str.length();
                if (i11 != str2.length()) {
                    throw new RuntimeException();
                }
                String[] strArr2 = A0m;
                strArr2[1] = "SQxDsveS3BVeMl0nUnJayKxDlwKiSRwM";
                strArr2[3] = "CPUPH8uU8pe6dd8URnD7AGDEQ4lKFlLs";
                A0o(list, childAt);
            }
            return;
        }
        list.add(view);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public boolean A0p() {
        return A18() == EnumC0466Iw.A05 || A18() == EnumC0466Iw.A03;
    }

    /* JADX INFO: Access modifiers changed from: private */
    public boolean A0q() {
        return A0F().A0h();
    }

    public final long A0x() {
        return this.A00;
    }

    @Nullable
    public final ZO A0y() {
        return this.A0a;
    }

    @Nullable
    public final AnonymousClass18 A0z() {
        return A0F().A0D();
    }

    public final C6G A10() {
        return this.A0b;
    }

    public final C0823Wy A11() {
        return this.A0c;
    }

    @Nullable
    public final TD A12() {
        return this.A0F;
    }

    @Nullable
    /* JADX INFO: renamed from: A13, reason: merged with bridge method [inline-methods] */
    public final C0461Ir getAdCoverImage() {
        return A0F().A0F();
    }

    @Nullable
    /* JADX INFO: renamed from: A14, reason: merged with bridge method [inline-methods] */
    public final C0461Ir getAdIcon() {
        return A0F().A0G();
    }

    @Nullable
    public final TA A15() {
        return this.A0G;
    }

    @Nullable
    public final C0463It A16() {
        return this.A0H;
    }

    public final EnumC0464Iu A17() {
        return this.A0I;
    }

    public final EnumC0466Iw A18() {
        return A0F().A0I();
    }

    public final L6 A19() {
        return this.A0f;
    }

    @Nullable
    public final C0640Pt A1A() {
        return this.A0R;
    }

    @Nullable
    public final String A1B() {
        if (this.A0a == null || !isAdLoaded()) {
            return null;
        }
        return this.A0a.A63();
    }

    @Nullable
    public final String A1C() {
        return this.A0T;
    }

    @Nullable
    public final String A1D() {
        return A0G(true).A0O();
    }

    @Nullable
    public final String A1E() {
        return A0G(true).A0P();
    }

    @Nullable
    public final String A1F() {
        return A0F().A0c();
    }

    @Nullable
    public final String A1G() {
        ZO zo = this.A0a;
        if (zo == null || TextUtils.isEmpty(zo.A0E().A0d())) {
            return null;
        }
        return this.A0b.A0S(this.A0a.A0E().A0d());
    }

    @Nullable
    public final List<TB> A1H() {
        if (this.A0a == null || !isAdLoaded()) {
            return null;
        }
        return this.A0a.A0H();
    }

    public final void A1I() {
        if (!C2E.A00(this.A0c.A00()).A0O(this.A0c, false)) {
            A0a();
            return;
        }
        C0823Wy c0823Wy = this.A0c;
        M9 m9A01 = MA.A01(c0823Wy, c0823Wy.A08(), A1B(), this.A07);
        if (m9A01 == null) {
            A0a();
        } else {
            ((XQ) this.A07.getNativeAdLayoutApi()).A03(m9A01);
            m9A01.A0K();
        }
    }

    public final void A1J(@Nullable Drawable drawable) {
        this.A01 = drawable;
        A1c(drawable != null, true);
        String[] strArr = A0m;
        if (strArr[4].length() == strArr[6].length()) {
            throw new RuntimeException();
        }
        String[] strArr2 = A0m;
        strArr2[4] = "Q96JEqttX0oIb74l";
        strArr2[6] = "mqYIcYUMJzEb";
    }

    public final void A1K(View view) {
        this.A0i.add(view);
        view.setOnClickListener(this.A0F);
        view.setOnTouchListener(this.A0F);
        if (Build.VERSION.SDK_INT >= 18) {
            boolean zA0z = ID.A0z(view.getContext());
            String[] strArr = A0m;
            if (strArr[1].charAt(16) == strArr[3].charAt(16)) {
                throw new RuntimeException();
            }
            String[] strArr2 = A0m;
            strArr2[4] = "GeTsVetQiODT9yUx";
            strArr2[6] = "UPeG8QvAHkrx";
            if (zA0z) {
                view.setOnLongClickListener(this.A0F);
            }
        }
    }

    public final void A1L(View view, ImageView imageView) {
        ArrayList arrayList = new ArrayList();
        A0o(arrayList, view);
        A0f(view, imageView, arrayList, true);
    }

    public final void A1M(View view, ImageView imageView, List<View> clickableViews) {
        A0f(view, imageView, clickableViews, true);
    }

    public final void A1N(View view, AdNativeComponentView adNativeComponentView) {
        ArrayList arrayList = new ArrayList();
        A0o(arrayList, view);
        A0f(view, adNativeComponentView, arrayList, false);
    }

    public final void A1O(View view, AdNativeComponentView adNativeComponentView, List<View> clickableViews) {
        A0f(view, adNativeComponentView, clickableViews, false);
    }

    public final void A1P(MediaView mediaView) {
        if (mediaView != null) {
            this.A0Y = true;
        }
    }

    public final void A1Q(MediaView mediaView) {
        if (mediaView != null) {
            this.A0Z = true;
        }
    }

    public final void A1R(NativeAdBase nativeAdBase, NativeAdListener nativeAdListener) {
        if (nativeAdListener == null) {
            return;
        }
        A0m(new A4(nativeAdListener, nativeAdBase));
    }

    public final void A1S(@Nullable NativeAdLayout nativeAdLayout) {
        this.A07 = nativeAdLayout;
    }

    public final void A1T(ZO zo) {
        A0h(zo, true);
        if (this.A0G != null) {
            List<TB> listA0H = zo.A0H();
            String[] strArr = A0m;
            if (strArr[7].length() != strArr[5].length()) {
                throw new RuntimeException();
            }
            String[] strArr2 = A0m;
            strArr2[0] = "OfB2JsewzcYgCxMvn97l0AOaUV28WOmd";
            strArr2[2] = "fN7XzjPm8qHCAEGwTiZWrbsVWAwnjwvJ";
            if (listA0H != null) {
                TK tk2 = new TK(this);
                Iterator<TB> it = zo.A0H().iterator();
                while (it.hasNext()) {
                    it.next().A0i(tk2);
                }
            }
        }
    }

    public final void A1U(@Nullable C0832Xh c0832Xh) {
        this.A0U = new WeakReference<>(c0832Xh);
    }

    public final void A1V(EnumC0455Il enumC0455Il, @Nullable String str, C6L c6l) {
        if (str == null) {
            this.A0c.A0D().A2d();
        } else {
            this.A0c.A0D().A2c();
        }
        this.A00 = System.currentTimeMillis();
        if (this.A0j) {
            AdSettings.IntegrationErrorMode integrationErrorModeA00 = C00511r.A00(this.A0c);
            String strA0W = A0W(566, 30, R.styleable.AppCompatTheme_windowFixedHeightMinor);
            AdSettings.IntegrationErrorMode integrationErrorMode = AdSettings.IntegrationErrorMode.INTEGRATION_ERROR_CRASH_DEBUG_MODE;
            if (!integrationErrorMode.equals(integrationErrorModeA00)) {
                J3 j32 = new J3(AdErrorType.LOAD_AD_CALLED_MORE_THAN_ONCE, A0W(566, 30, R.styleable.AppCompatTheme_windowFixedHeightMinor));
                A11().A0D().A2a(L5.A01(this.A00), j32.A03().getErrorCode(), j32.A04());
                TA ta = this.A0G;
                if (ta != null) {
                    ta.AAc(j32);
                } else {
                    Log.e(A0W(76, 17, 41), strA0W);
                }
                C01897m c01897m = new C01897m(strA0W);
                A11().A06().A8u(A0W(550, 3, 3), C01887l.A0c, c01897m);
            } else {
                throw new C00561w(strA0W);
            }
        }
        this.A0j = true;
        this.A0E = enumC0455Il;
        if (enumC0455Il.equals(EnumC0455Il.A05)) {
            this.A08 = EnumC00330z.A05;
        }
        this.A0C = c6l;
        C00491p c00491p = new C00491p(this.A0g, this.A0J, A0R(), null, 1);
        c00491p.A05(enumC0455Il);
        c00491p.A06(this.A0S);
        c00491p.A07(this.A0T);
        this.A0A = new EB(this.A0c, c00491p);
        this.A0A.A0M(new TL(this));
        this.A0A.A0P(str);
    }

    public final void A1W(@Nullable C0463It c0463It) {
        this.A0H = c0463It;
    }

    public final void A1X(EnumC0464Iu enumC0464Iu) {
        this.A0I = enumC0464Iu;
    }

    public final void A1Y(J8 j82) {
        if (!A0q()) {
            if (J8.A04.equals(j82)) {
                C0823Wy c0823Wy = this.A0c;
                String[] strArr = A0m;
                if (strArr[7].length() != strArr[5].length()) {
                    throw new RuntimeException();
                }
                String[] strArr2 = A0m;
                strArr2[7] = "er71GFUENUkH6mVEmd80tsw";
                strArr2[5] = "eRFgtOOg0hnVr16Te1PNaNs";
                c0823Wy.A0D().A2g(AdPlacementType.NATIVE_BANNER.toString(), this.A0g);
            } else {
                this.A0c.A0D().A2g(AdPlacementType.NATIVE.toString(), this.A0g);
            }
        }
        this.A0J = j82;
    }

    public final void A1Z(AbstractC0639Ps abstractC0639Ps) {
        this.A0V = new WeakReference<>(abstractC0639Ps);
    }

    public final void A1a(boolean z3) {
        this.A0W = z3;
    }

    public final void A1b(boolean z3) {
        this.A0X = z3;
    }

    /* JADX WARN: Removed duplicated region for block: B:22:0x006d  */
    /* JADX WARN: Removed duplicated region for block: B:35:0x00e7  */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
        To view partially-correct code enable 'Show inconsistent code' option in preferences
    */
    public final void A1c(boolean r7, boolean r8) {
        /*
            Method dump skipped, instruction units count: 246
            To view this dump change 'Code comments level' option to 'DEBUG'
        */
        throw new UnsupportedOperationException("Method not decompiled: com.facebook.ads.redexgen.X.TB.A1c(boolean, boolean):void");
    }

    public final boolean A1d() {
        return this.A07 == null;
    }

    @Override // com.facebook.ads.redexgen.X.InterfaceC0469Iz
    public final int A6C() {
        AdNativeComponentView adNativeComponentView = this.A06;
        if (adNativeComponentView instanceof AdNativeComponentView) {
            View videoView = adNativeComponentView.getAdContentsView();
            if (videoView instanceof P0) {
                return ((P0) videoView).getCurrentPosition();
            }
            return -1;
        }
        return -1;
    }

    public final NativeAdBase.NativeAdLoadConfigBuilder buildLoadAdConfig(NativeAdBase nativeAdBase) {
        return new C0467Ix(this, nativeAdBase);
    }

    public final void destroy() {
        JH.A05(A0W(553, 7, 7), A0W(363, 19, 23), A0W(21, 8, 126));
        if (ID.A1P(this.A0c)) {
            A11().A09().AF5(this.A03);
        }
        if (!A0q()) {
            this.A0c.A0D().A2h();
        }
        EB eb2 = this.A0A;
        if (eb2 != null) {
            eb2.A0R(true);
            this.A0A = null;
        }
    }

    public final void downloadMedia() {
        if (this.A0E.equals(EnumC0455Il.A05)) {
            this.A08 = EnumC00330z.A04;
        }
        this.A0E = EnumC0455Il.A04;
        A0h(this.A0a, false);
    }

    @Nullable
    public final String getAdBodyText() {
        return A0G(true).A0J();
    }

    @Nullable
    public final String getAdCallToAction() {
        return A0G(true).A0X();
    }

    @Nullable
    public final String getAdChoicesImageUrl() {
        if (getAdChoicesIcon() == null) {
            return null;
        }
        C0461Ir adChoicesIcon = getAdChoicesIcon();
        String[] strArr = A0m;
        if (strArr[0].charAt(5) == strArr[2].charAt(5)) {
            throw new RuntimeException();
        }
        String[] strArr2 = A0m;
        strArr2[7] = "MRTmAh7YnfwjJHGdoro5GGF";
        strArr2[5] = "5NAjexdbITBZf7E7VHPXfXw";
        return adChoicesIcon.getUrl();
    }

    @Nullable
    public final String getAdChoicesLinkUrl() {
        return A0F().A0K();
    }

    @Nullable
    public final String getAdChoicesText() {
        return A0F().A0L();
    }

    @Nullable
    public final String getAdHeadline() {
        return A0G(true).A0M();
    }

    @Nullable
    public final String getAdLinkDescription() {
        return A0G(true).A0N();
    }

    @Nullable
    public final String getAdSocialContext() {
        return A0G(true).A0R();
    }

    @Nullable
    public final String getAdTranslation() {
        return A0G(true).A0U();
    }

    @Nullable
    public final String getAdUntrimmedBodyText() {
        return A0G(true).A0V();
    }

    @Nullable
    public final String getAdvertiserName() {
        return A0G(true).A0W();
    }

    public final float getAspectRatio() {
        ZO zo = this.A0a;
        if (zo != null) {
            AnonymousClass11 anonymousClass11A0E = zo.A0E();
            String[] strArr = A0m;
            if (strArr[7].length() != strArr[5].length()) {
                throw new RuntimeException();
            }
            String[] strArr2 = A0m;
            strArr2[1] = "Gk0JtmCCXYpL8dPDyrRdSPBqQO7pSw2M";
            strArr2[3] = "hZFl2ZrFl9W40ZKmdwMt57qcTxbjzHrP";
            C0461Ir nativeAdImage = anonymousClass11A0E.A0F();
            if (nativeAdImage != null) {
                int width = nativeAdImage.getWidth();
                int height = nativeAdImage.getHeight();
                if (height > 0) {
                    return width / height;
                }
                return 0.0f;
            }
        }
        return 0.0f;
    }

    @Nullable
    public final String getId() {
        if (!isAdLoaded()) {
            return null;
        }
        String str = this.A0h;
        String[] strArr = A0m;
        if (strArr[4].length() == strArr[6].length()) {
            throw new RuntimeException();
        }
        String[] strArr2 = A0m;
        strArr2[7] = "iawtn5ImZtHDfKUJHK1ePg9";
        strArr2[5] = "61vZsTsWUXSHOR043CjOsZn";
        return str;
    }

    public final String getPlacementId() {
        return this.A0g;
    }

    @Nullable
    public final Drawable getPreloadedIconViewDrawable() {
        C0461Ir c0461IrA0G;
        Bitmap bitmapA0M;
        ZO adapter = this.A0a;
        if (adapter != null && (c0461IrA0G = A0F().A0G()) != null && (bitmapA0M = this.A0b.A0M(c0461IrA0G.getUrl())) != null) {
            C0823Wy c0823WyA11 = A11();
            boolean zA1d = A1d();
            String[] strArr = A0m;
            if (strArr[0].charAt(5) == strArr[2].charAt(5)) {
                throw new RuntimeException();
            }
            String[] strArr2 = A0m;
            strArr2[4] = "5rXv1LFjMRKkpcU7";
            strArr2[6] = "SFgDKFOuseBk";
            return A05(c0823WyA11, bitmapA0M, zA1d, A1C());
        }
        return null;
    }

    @Nullable
    public final String getPromotedTranslation() {
        return A0G(true).A0Q();
    }

    @Nullable
    public final String getSponsoredTranslation() {
        return A0G(true).A0S();
    }

    public final boolean hasCallToAction() {
        ZO zo = this.A0a;
        return zo != null && zo.A0S();
    }

    public final boolean isAdInvalidated() {
        boolean zA0A = true;
        EB eb2 = this.A0A;
        if (eb2 != null) {
            zA0A = eb2.A0S();
        } else {
            C0859Yk c0859Yk = this.A0B;
            String[] strArr = A0m;
            if (strArr[1].charAt(16) == strArr[3].charAt(16)) {
                throw new RuntimeException();
            }
            String[] strArr2 = A0m;
            strArr2[7] = "v5uFzvuQ32LyebguaenlI2Y";
            strArr2[5] = "nsJGpwd0QPZOfVeZMyNRGWR";
            if (c0859Yk != null) {
                zA0A = c0859Yk.A0A();
            }
        }
        this.A0c.A0D().A4Y(zA0A);
        return zA0A;
    }

    public final boolean isAdLoaded() {
        ZO zo = this.A0a;
        return zo != null && zo.A0T();
    }

    public final void loadAd() {
        JH.A05(A0W(560, 6, 14), A0W(382, 24, 98), A0W(13, 8, 89));
        A1V(EnumC0455Il.A00(NativeAdBase.MediaCacheFlag.ALL), null, new C6L(false, -1, -1));
    }

    public final void loadAd(NativeAdBase.NativeLoadAdConfig nativeLoadAdConfig) {
        JH.A05(A0W(560, 6, 14), A0W(382, 24, 98), A0W(5, 8, 76));
        ((C0467Ix) nativeLoadAdConfig).A00();
    }

    public final void onCtaBroadcast() {
        View view = this.A05;
        if (view != null) {
            view.performClick();
        }
    }

    public final void repair(Throwable th2) {
        View view = this.A04;
        if (view != null) {
            view.post(new TM(this));
        }
        String str = A0W(164, 16, 64) + C0514Kw.A03(this.A0c, th2);
        A11().A0D().A2a(L5.A01(this.A00), 2001, str);
        TA ta = this.A0G;
        if (ta != null) {
            ta.AAc(new J3(2001, str));
        }
    }

    public final void setExtraHints(ExtraHints extraHints) {
        if (extraHints == null) {
            return;
        }
        A0n(extraHints.getHints());
        this.A0T = extraHints.getMediationData();
    }

    public final void setOnTouchListener(View.OnTouchListener onTouchListener) {
        this.A02 = onTouchListener;
    }

    /* JADX WARN: Removed duplicated region for block: B:22:0x0072  */
    /* JADX WARN: Removed duplicated region for block: B:31:0x00bd  */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
        To view partially-correct code enable 'Show inconsistent code' option in preferences
    */
    public final void unregisterView() {
        /*
            Method dump skipped, instruction units count: 303
            To view this dump change 'Code comments level' option to 'DEBUG'
        */
        throw new UnsupportedOperationException("Method not decompiled: com.facebook.ads.redexgen.X.TB.unregisterView():void");
    }
}
